...
GZ_REGISTER_MODEL_PLUGIN(LiftDragPluginWithWind)

/////////////////////////////////////////////////
LiftDragPluginWithWind::LiftDragPluginWithWind() : cla(1.0), cda(0.01), cma(0.01), rho(1.2041) {
    ...
    this->windVelocity = ignition::math::Vector3d(0.0, 0.0, 0.0);
...
/////////////////////////////////////////////////
LiftDragPluginWithWind::~LiftDragPluginWithWind() {
}

/////////////////////////////////////////////////
void LiftDragPluginWithWind::Load(physics::ModelPtr _model,
                                  sdf::ElementPtr _sdf) {
    GZ_ASSERT(_model, "LiftDragPluginWithWind _model pointer is NULL");
    GZ_ASSERT(_sdf, "LiftDragPluginWithWind _sdf pointer is NULL");
    this->model = _model;
    this->sdf = _sdf;
    
    this->world = this->model->GetWorld();
    GZ_ASSERT(this->world, "LiftDragPluginWithWind world pointer is NULL");
    
    this->physics = this->world->Physics();
    GZ_ASSERT(this->physics, "LiftDragPluginWithWind physics pointer is NULL");
    ...
    if (_sdf->HasElement("link_name")) {
        ...
        if (!this->link) {
            gzerr << "Link with name[" << linkName << "] not found. "
                  << "The LiftDragPluginWithWind will not generate forces\n";
        } else {
            this->updateConnection = event::Events::ConnectWorldUpdateBegin(
                    std::bind(&LiftDragPluginWithWind::OnUpdate, this));
        }
    }
...
#endif
    this->windSubscriber = this->node
                               ->Subscribe("~/wind", &LiftDragPluginWithWind::windVelocityCallback,
                                       this);
}

/////////////////////////////////////////////////
void LiftDragPluginWithWind::OnUpdate() {
    GZ_ASSERT(this->link, "Link was NULL");
    // get linear velocity at cp in inertial frame
    ignition::math::Vector3d vel = this->link->WorldLinearVel(this->cp) - windVelocity;
...
void LiftDragPluginWithWind::windVelocityCallback(ConstWindPtr &msg) {
    if (msg->enable_wind() && link->WindMode()) {
        gazebo::msgs::Vector3d windLinearVelocity = msg->linear_velocity();
        this->windVelocity = ignition::math::Vector3d(
                windLinearVelocity.x(), windLinearVelocity.y(), windLinearVelocity.z());
    } else {
